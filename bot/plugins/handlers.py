...
async def file_setting_manager(bot, msg):
    ...
    if not msg.text :
        ...
        setting = con.setting
        for channel in setting.backup_channels :
            backup = await msg.copy(channel.chat_id)
            file_channel = con.file_channel(chat_id=channel.chat_id , message_id=backup.id)
            if file_channel :
                file_channels.append(file_channel.id)

        user_chat_id = msg.from_user.id 
        file_ids = await utils.get_file_ids(backup)
        unique_id_hash = utils.get_hash(backup)
        name = utils.get_name(backup)
...
async def call_file_setting_manager(bot , call  ):
    ...
    file_data = con.file(unique_id_hash = unique_id_hash   , user_chat_id=call.from_user.id)
    ...
    if file_data :
        ...
        if status == 'quality'  :
            file_data = con.file(unique_id_hash = unique_id_hash  ,quality=data  , user_chat_id=call.from_user.id)
            
        elif status == 'sub' :
            file_data = con.file(unique_id_hash = unique_id_hash  ,subtitle_status=data  , user_chat_id=call.from_user.id)
